# -*- coding: utf-8 -*-
"""
Created on Thu Apr 21 16:27:01 2016
...
def finetune_log_layer_sgd(
    ...
    sda.logLayer.train_cost_array = [] 
    sda.logLayer.train_error_array = []
    
    best_attempt_cost = numpy.inf
    best_attempt_error = numpy.inf    
    best_model_name = 'best_sda.pkl'
    
    # save clean model
    os.chdir(init_folder)
    init_da_name = 'init_pretrained.pkl'
    with open(init_da_name, 'wb') as f:
        pickle.dump(sda, f)
    os.chdir('../')
    ...
    train_fn = build_finetune_functions(
        sda=sda,
        batch_size=batch_size,
    ...
    for cur_attempt in xrange(n_attempts):        
        ...
        # open clean model
        os.chdir(init_folder)
        sda = pickle.load(open(init_da_name))
        os.chdir('../')
        ...
        while (global_index < global_epochs) and (not done_global_loop):
            ...
            while (pat_num < train_reader.n_files) and (not done_global_loop):
                ...
                validation_frequency = 2 * n_train_batches
                validation_increase = 1.2
                if validation_frequency == 0:
                        validation_frequency = 2 * n_train_batches
                ...
                while (pat_epoch < pat_epochs) and (not done_looping):
                    ...
                    for batch_index in xrange(n_train_batches):          
                        ...
                        cur_epoch_cost.append(sample_cost)
                        cur_epoch_error.append(sample_error)                        
                        ...
                        if (iter + 1) % validation_frequency == 0:
                            ...
                            if sample_cost < best_cost:
                                ...
                                #improve patience if loss improvement is good enough
                                if sample_cost < best_cost * improvement_threshold:
                                    validation_frequency = int(validation_frequency * \
                                    validation_increase)
                                    if validation_frequency == 0:
                                        validation_frequency = n_train_batches
                    ...
                    sda.logLayer.train_cost_array[-1].append(mean_cost)
                    
                    mean_error = numpy.mean(cur_epoch_error)                              
                    sda.logLayer.train_error_array.append([])
                    sda.logLayer.train_error_array[-1].append(float(sda.logLayer.epoch))
                    sda.logLayer.train_error_array[-1].append(mean_error)
                
                attempt_cost = numpy.concatenate((attempt_cost, cur_epoch_cost))
                attempt_error = numpy.concatenate((attempt_error, cur_epoch_error))
                            
            gc.collect()
                                                              
        mean_attempt_cost = numpy.mean(attempt_cost)
        mean_attempt_error = numpy.mean(attempt_error)         
        
        os.chdir(debug_folder)
        f = open(debug_file, 'a')
        f.write('cur_attempt %i, ' % cur_attempt)
        f.write('mean_attempt_cost %f\n' % mean_attempt_cost)
        f.write('mean_attempt_error %f\n' % mean_attempt_error)
        f.close()
        os.chdir('../')
        
        if best_attempt_error > mean_attempt_error:
            best_attempt_cost = mean_attempt_cost
            best_attempt_error = mean_attempt_error
            sda.logLayer.best_cost = best_attempt_cost
            sda.logLayer.best_error = best_attempt_error                

            #save the best model for sda
            os.chdir(debug_folder)
            with open(best_model_name, 'wb') as f:
                pickle.dump(sda, f)
            os.chdir('../')
    ...
    # load the best model in sda
    os.chdir(debug_folder)
    sda = pickle.load(open(best_model_name))
    os.chdir('../')
